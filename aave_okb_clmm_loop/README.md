...
The Almanak SDK's built-in backtesting engine was used to run a 90-day
historical simulation and a full 63-configuration parameter sweep.
Because the backtester replays the exact same `IntentStrategy` class
that runs live on mainnet, the backtest results map directly to
expected on-chain performance — no translation layer, no parallel
implementation to maintain.

```bash
# Default PnL backtest (90 days, $1000 capital)
almanak strat backtest pnl -d aave_okb_clmm_loop --days 90

# Full parameter sweep (tests 63 configurations)
...
# Paper trading on Anvil fork (executes intents against simulated liquidity)
...
```bash
# Reproduce the 90-day PnL backtest
almanak strat backtest pnl -d aave_okb_clmm_loop --days 90

# Full parameter sweep (63 configs)
almanak strat backtest sweep -d aave_okb_clmm_loop --days 90

# Test at $10k scale
almanak strat backtest pnl -d aave_okb_clmm_loop --capital 10000 --days 90

# Save sweep results to JSON
almanak strat backtest sweep -d aave_okb_clmm_loop --days 90 --output results.json

# Paper trading on Anvil fork (real intent execution against simulated liquidity)
almanak strat backtest paper -d aave_okb_clmm_loop
```
