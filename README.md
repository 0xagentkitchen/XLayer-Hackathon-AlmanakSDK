...
historical simulation with a 63-configuration parameter sweep** using
the **Almanak SDK's built-in backtesting engine**
(`almanak strat backtest sweep`) and used the results to tune the
adaptive rebalance engine.

The SDK's backtester loads the same `IntentStrategy` class that runs
on mainnet, replays historical price data through the `decide()` loop,
and tracks realized P&L across all protocol legs. Because the whole
thing runs against the exact same strategy code that executes live,
the backtest results map directly to expected on-chain performance —
no translation layer, no parallel implementation to maintain.
...
- **Engine**: Almanak SDK built-in (`almanak strat backtest sweep`)
...
- **Adaptive engine**: range sizing + confirmation cycles + cooldown gate
  (identical to live strategy — same code path)
...
```bash
# 90-day PnL backtest (default config)
almanak strat backtest pnl -d aave_okb_clmm_loop --days 90

# Full 63-config parameter sweep (across vol_multiplier × confirmation × cooldown)
almanak strat backtest sweep -d aave_okb_clmm_loop --days 90

# Custom capital / period
almanak strat backtest pnl -d aave_okb_clmm_loop --capital 10000 --days 180

# Paper trading on Anvil fork (full intent execution with simulated liquidity)
almanak strat backtest paper -d aave_okb_clmm_loop
```

See [`aave_okb_clmm_loop/README.md`](aave_okb_clmm_loop/README.md#backtesting-90-day-historical-simulation)
for the full technical breakdown including the naive-vs-adaptive
comparison table, scaling considerations, and IL caveats.
...
almanak strat backtest sweep -d strategies/xlayer/xlayer_deterministic/aave_okb_clmm_loop --days 90
```
...
4. **SDK-native backtesting**: 90-day historical simulation with
   63-configuration parameter sweep via `almanak strat backtest sweep`.
   Best config: +106% APR over the test period
